...
## v0.5.319 — fix(harness): daemon 多客户端并发 + provider 段名 TOML 转义
**Date**: 2026-09-20
**Commit**: _(see `git log -1 v0.5.319`)_
**Tag**: v0.5.319
**Test status**: TapgoTests 3455 通过 / 12 失败（均为既有 SSH/auth.json 环境失败）；daemon 并发回归测试旧代码必失败、新代码 8 断言全过；TomlKey 14 断言；本机真实回合 + 「长任务并发普通对话」回归通过；chenlaiyi-mbp 遗留注册表触发的 TOML 回归已修待复测
**Changed**:
...
- Sources/TapgoTests/HarnessDaemonConcurrencyTests.swift (新增) + TestMain 注册: 用真实 TapgoHarness + 假 codex 验证 A 连接占线时 B/C 仍能拿到 initialize 响应。
- scripts/install-harness-daemon.sh 用法不变；本机与 jkmacmini 已重装/热切换新 daemon 二进制。
...
**Next**: 把 daemon 版本纳入 fleet 部署脚本（daemon 需单独 install-harness-daemon.sh）；长任务并发的人工回归。
...
**Tag**: v0.5.314
**Test status**: — 3488 passed, 0 failed —
**Changed**:
- 新增 scripts/evolution-drift.py（EVO-055）作为漂移真源：releasesBehind（结构化记录里 running < v <= installed 的条数，running 早于最早记录时给 null 而非偏小的错数）、firstSeenAt 跨轮继承、seenRuns 按 run-id 去重（一轮内多次写 state 只算一次）、remediation 与一行可读文案。
- evolve.sh 接入：write_state 计算并写进 state.localApp.drift（installed/running/stale 保持原位，派生元数据收在 drift 子对象；此时当前版本记录已落盘，计数才准）；漂移时把重启命令作为第一条 nextActions；总结行升级为「运行 x / 已安装 y（落后 N 个版本，自 … 起，已累计 K 轮）」。helper 不可用时回退旧形状，发布流程不会因它中断。
- 新增 check 子命令：python3 scripts/evolution-drift.py check 一行查询，退出码表达（漂移 1 / 一致 0 / state 不可读 2）；旧 state 形状（无 drift 块）降级成通用提示行而不是误判为已对齐。
- 新增 scripts/tests/evolution-drift-test.sh 27 项断言（一致/落后、计数、记录不覆盖时 null、跨轮继承、同 run-id 去重、无进程降级、旧形状降级、state 不可读退出 2）；失败注入新增 S36 覆盖 evolve.sh → state 的集成（漂移元数据、待办动作、WARN），全套 169 项；两者接入 run-all.sh。
- 边界：漂移元数据是只读计算，不重启任何进程——重启会终止当前会话，收敛动作仍由操作者执行；本轮保证的是它不会被忘记、有年龄、可被下一轮自动捞起。

漂移从一次性 WARN 变成跨轮延续的待办
**Why**: 三轮发布下来，本机运行进程一直停在 0.5.299，而机制只会在发布日志里叹一口气：state 里一个 stale 布尔不会老化、不能随时查询、下一轮就没人记得。让它有 firstSeenAt 和 seenRuns，就是把「记得住」从人的注意力换成机器的状态。
**Next**: EVO-021 操作者模型基线: 用真实 runner 跑 3 个任务，记录首份 model_eval_history 与成本（待操作者提供 runner）


## v0.5.313 — test(evolution): H5 渲染执行级测试
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.313`)_
**Tag**: v0.5.313
**Test status**: — 3485 passed, 0 failed —
**Changed**:
...
手机端自进化卡片渲染纳入秒级门禁
**Why**: 字符串断言只能证明键名写对了，证明不了条件成立、分支走对、文案算对。H5 是一千多行真实 UI，此前一次都没被执行过——EVO-053 抓的是形状错配，形状对了之后「条件永不成立」「边界写成 >=」仍然只能靠人在真机上看。让真实 app.js 在 node 里跑起来，这类错误就变成秒级门禁。
**Next**: EVO-021 操作者模型基线: 用真实 runner 跑 3 个任务，记录首份 model_eval_history 与成本（待操作者提供 runner）
...
**Tag**: v0.5.311
**Test status**: — 3464 passed, 0 failed —
**Changed**:
...
## v0.5.310 — test(evolution): 维护告警 launchd 自检
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.310`)_
**Tag**: v0.5.310
**Test status**: — 3444 passed, 0 failed —
**Changed**:
- 新增 scripts/evolution-maintenance-selftest.sh：装独立 Label 的临时 LaunchAgent（RunAtLoad），通过 EnvironmentVariables 注入必失败演练桩 + 通知捕获 + 临时 state/日志，跑完 bootout 并删除 plist——生产任务与 state 不受影响。
- 真机验证：launchd 上下文里维护按预期失败（last exit code=1）、历史写 status=failed 且带注入原因、通知脚本收到标题与完整原因；--use-system-notify 变体也跑通。
- 这段组合此前从没跑过：launchd+成功已验、直接调用+失败+假 osascript 已验，唯独「launchd + 失败 → 真实告警」缺失。
- 自检脚本修掉一个抢跑 bug：维护脚本先写历史、后发通知，原来只等历史就断言导致偶发假失败；现在等 launchd 不再 running 再断言。
- 新增 16 项回归（plist 形状 8 + 真实 launchd 运行 8，无 launchctl 自动跳过），接入 run-all 与 benchmark。

真实 launchd 跑一次失败维护并验证通知
**Why**: 告警链路是维护任务唯一的失败出口：静默失败等于没人知道可恢复性已经失效。EVO-051 把这条最危险的组合变成可重复运行、可验证的一条命令。
**Next**: EVO-021 操作者模型基线: 用真实 runner 跑 3 个任务，记录首份 model_eval_history 与成本（待操作者提供 runner）


## v0.5.309 — test(evolution): canary 全链路演练
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.309`)_
**Tag**: v0.5.309
**Test status**: — 3441 passed, 0 failed —
**Changed**:
...
**Tag**: v0.5.307
**Test status**: — 3435 passed, 0 failed —
**Changed**:
...
## v0.5.306 — fix(evolution): 指标摘要契约
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.306`)_
**Tag**: v0.5.306
**Test status**: — 3432 passed, 0 failed —
**Changed**:
...
## v0.5.305 — feat(evolution): 手机端指标补齐
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.305`)_
**Tag**: v0.5.305
**Test status**: — 3429 passed, 0 failed —
**Changed**:
...
**Why**: EVO-036/043/047 造出的指标此前只在桌面端可见：手机端卡片只有 benchmark/model/backlog 与几行状态，长尾周期、恢复速度与单轮成本对用户不可见。
**Next**: EVO-021 操作者模型基线: 用真实 runner 跑 3 个任务，记录首份 model_eval_history 与成本（待操作者提供 runner）


## v0.5.304 — test(evolution): canary 提升演练
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.304`)_
**Tag**: v0.5.304
**Test status**: — 3416 passed, 0 failed —
**Changed**:
...
## v0.5.303 — fix(evolution): 部署参数经 ssh 拼接失真
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.303`)_
**Tag**: v0.5.303
**Test status**: — 3413 passed, 0 failed —
**Changed**:
...
- 演练升级：fake ssh 忠实模拟参数拼接，新增哨兵静态断言、非 .app 路径拒绝、ssh 参数留痕；测试 32→42 项。
- 安全护栏：演练在临时目录外直接拒绝运行，并断言所有安装目标都在 TMP 内（测试曾尝试写真实 /Applications，现已不可能）。
- 真机复验：v0.5.302 补跑部署 → 两台远端 PID 正常且 UI ASSERT OK。

哨兵值 + 远端守卫，真机部署恢复正常
**Why**: 真机暴露 ssh 会把远端参数用空格拼成一条命令：空参数消失导致后续参数前移，v0.5.302 的部署因此把 App 装到名为 3 的目录并报 PID=none（发布已公开、三机却没更新）。
**Next**: EVO-021 操作者模型基线: 用真实 runner 跑 3 个任务，记录首份 model_eval_history 与成本（待操作者提供 runner）


## v0.5.302 — test(evolution): 部署端到端演练
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.302`)_
**Tag**: v0.5.302
**Test status**: — 3410 passed, 0 failed —
**Changed**:
...
## v0.5.301 — feat(evolution): UI 快照基线比对
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.301`)_
**Tag**: v0.5.301
**Test status**: — 3407 passed, 0 failed —
**Changed**:
...
**Tag**: v0.5.299
**Test status**: — 3401 passed, 0 failed —
**Changed**:
...
## v0.5.298 — fix(evolution): state schema 版本漂移
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.298`)_
**Tag**: v0.5.298
**Test status**: — 3388 passed, 0 failed —
**Changed**:
...
## v0.5.297 — feat(evolution): 成本归属
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.297`)_
**Tag**: v0.5.297
**Test status**: — 3385 passed, 0 failed —
**Changed**:
- 新增 TapgoCore.EvolutionCostSnapshot：state/evolution_cost.json 的写入/读取契约（原子写、缺失或非法 JSON 返回 nil），只记能核实的累计 token，不估算美元金额。
- SessionStore 在自进化线程（evolutionMode 或 evo- 前缀）收到 turnCompleted/tokenUsageUpdated 时写入累计 usageTotal：同一值不重写、5 秒内且增量不足 1000 跳过，写失败不影响会话主流程。
- evolve.sh 新增 resolve_run_cost：EVOLVE_RUN_TOKENS/EVOLVE_RUN_COST_USD 优先（source=env）→ 否则读 App 快照并与上次发布记录 tokens 相减得到本轮增量（source=app-snapshot-delta）→ 都没有就留空不编造；日志打印 Run cost source 便于审计。
- state schema 升到 v4：新增 costSource 字段说明 tokens/costUSD 来自哪条路径。
- 测试：Swift 新增 14 项成本契约断言，失败注入扩到 145 项（快照增量归因、快照未更新不归因、env 优先）。

App 写自进化会话 token 快照，evolve.sh 取增量并标注来源
**Why**: EVO-036 建好了单轮 token/成本指标与 EVOLVE_RUN_TOKENS 通道，但没人往里写数据——指标长期为空等于没做。EVO-041 让 App 自己把自进化会话的用量交出来。
**Next**: EVO-042 反馈漏斗接入看板: 把 drafts/转化率/等待时长显示到 App 指标详情与手机端自进化卡片


## v0.5.296 — feat(evolution): 依赖路径探测
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.296`)_
**Tag**: v0.5.296
**Test status**: — 3368 passed, 0 failed —
**Changed**:
...
**Next**: EVO-041 自进化成本归属: App 把当前会话的 token/成本传给 evolve.sh（`EVOLVE_RUN_TOKENS`/`EVOLVE_RUN_COST_USD`），让单轮成本指标有真实数据


## v0.5.295 — feat(evolution): 三机界面自动断言
**Date**: 2026-09-13
**Commit**: _(see `git log -1 v0.5.295`)_
**Tag**: v0.5.295
**Test status**: — 3365 passed, 0 failed —
**Changed**:
...
**Tag**: v0.5.293
**Test status**: — 3359 passed, 0 failed —
**Changed**:
...
**Why**: 此前锁只能靠人记得 --break-remote-lock 才能从崩溃机器手里拿回来：机器断电后整个自进化闭环会被一把死锁堵住。EVO-037 让陈旧锁自愈，同时用元数据缺失保护避免误抢活锁。
**Next**: EVO-038 反馈闭环转化率: drafts → check → backlog → 版本的转化率与等待时长
...
**Tag**: v0.5.291
**Test status**: — 3344 passed, 0 failed —
**Changed**:
...
**Why**: 运行态文件由 6 个写入方产生，字段演进时 Python/Swift/H5 三处解析只能靠猜：旧代码遇到新字段会静默漏读。EVO-035 把版本契约显式化，并在写入侧硬门禁。
**Next**: EVO-036 度量扩展: 周期 P95、失败后 MTTR、单轮 token/时长成本


## v0.5.290 — feat(evolution): 发布前环境预检
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.290`)_
**Tag**: v0.5.290
**Test status**: — 3339 passed, 0 failed —
**Changed**:
...
## v0.5.289 — feat(evolution): 发布失败续跑
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.289`)_
**Tag**: v0.5.289
**Test status**: — 3336 passed, 0 failed —
**Changed**:
...
**Why**: 此前 push 成功但 release 失败时 tag 已存在，重跑 evolve.sh 会被 tag already exists 挡住，只能人工补 push/release/部署；EVO-033 让这一步变成一条 --resume 命令。
**Next**: EVO-034 发布前环境预检: gh 认证 / SDK / python3 / 磁盘 / 远端可达 / tag 冲突统一预检后再启动


## v0.5.288 — fix(evolution): 月度维护安装路径
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.288`)_
**Tag**: v0.5.288
**Test status**: — 3333 passed, 0 failed —
**Changed**:
...
**Why**: v0.5.287 只覆盖了 --print 路径，真实安装路径在运行时报 unbound variable；只跑一次的安装路径必须进回归，否则每个操作者第一次安装都会踩。
**Next**: EVO-033 发布失败续跑: `--resume` 从失败阶段继续（push 成功但 release 失败时不再整轮重来）


## v0.5.287 — feat(evolution): 月度维护自动化
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.287`)_
**Tag**: v0.5.287
**Test status**: — 3330 passed, 0 failed —
**Changed**:
...
**Tag**: v0.5.285
**Test status**: — 3306 passed, 0 failed —
**Changed**:
- 新增按月归档工具并在每轮收尾自动执行
- 归档回归与 benchmark archive-helper

过期 history 按月归档且不删除,保留近期记录
**Why**: history 持续追加,长期会拖慢读取并膨胀运行态目录
**Next**: EVO-031 远程触发回滚演练: H5/手机端提供只读“演练”按钮并显示最近结果
...
**Tag**: v0.5.283
**Test status**: — 3297 passed, 0 failed —
**Changed**:
- 支持 draft+单机 canary+promote 灰度发布
- deploy-fleet --only/--exclude 与 S16/S17 失败注入

draft release 先单机验证,通过后才发布 appcast 并部署其余机器
**Why**: 每版直接刷新 appcast 让所有客户端同时升级,坏包影响面是全量
**Next**: EVO-029 回滚演练: 定期从上一 tag 回滚并跑 health-check，验证可恢复性


## v0.5.282 — feat(evolution): 多机协调锁
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.282`)_
**Tag**: v0.5.282
**Test status**: — 3294 passed, 0 failed —
**Changed**:
- 新增远端 evolution-lock 原子互斥与持有者状态
- 冲突退出 9,显式 break 才能抢占,失败注入 S15

publish 前远端原子互斥,冲突显示持有者/时长,显式 break 才可抢占
**Why**: 各 Mac 本地锁无法防跨机同时演进,会撞版本和 tag
**Next**: EVO-028 发布 canary/灰度: 先只让一台客户端升级，观察后再全量 appcast


## v0.5.281 — feat(evolution): 反馈草稿 check 建议
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.281`)_
**Tag**: v0.5.281
**Test status**: — 3291 passed, 0 failed —
**Changed**:
- 新增草稿分类/测试 section 匹配/命令模板建议
- 建议仍只写 drafts,benchmark 纳入 suggest 幂等

自动匹配现有测试 section 并生成最小复现命令模板
**Why**: 草稿只有标题,人工仍需从零设计最小复现
...
**Tag**: v0.5.279
**Test status**: — 3269 passed, 0 failed —
**Changed**:
- 新增多 runner A/B 对比与退化门禁
- 共享预算/超时逻辑,回归扩到 13 项,backlog 顺序调整

同一任务集对比通过率/耗时/成本,候选低于基线即失败
**Why**: 单 runner 只有绝对分,无法判断模型/策略替换是否更好
**Next**: EVO-025 手机端自进化状态: PhoneRemote 状态快照暴露版本、进度、benchmark、模型评测与 backlog
...
**Tag**: v0.5.270
**Test status**: — 3226 passed, 0 failed —
**Changed**:
- v0.5.5 之前 11 节移入独立归档
- 补主日志/归档双向不变量并修复 MakeHistoryParity

主日志只保留 v0.5.5 之后,早期历史完整移入 evolution/archive/
**Why**: 主日志 274KB/3399 行,早期历史与现役版本混排,影响阅读与解析
**Next**: EVO-015 worktree 隔离: 分支隔离后的强化项，每轮在独立 git worktree 执行，主 checkout 零改动


## v0.5.269 — feat(evolution): iOS 版本序列分离
**Date**: 2026-09-12
**Commit**: _(see `git log -1 v0.5.269`)_
**Tag**: v0.5.269
**Test status**: — 3219 passed, 0 failed —
**Changed**:
- iOS 历史移入独立日志,主日志留指针
- 记录校验按 scope 分流并补 iOS 回归

iOS 1.0.x 独立归档,Mac 0.x 校验不再受跨序列重复干扰
**Why**: iOS 1.0.0 在 EVOLUTION.md 重复 3 次并与 Mac 0.x 共用版本解析,造成重复版本歧义
...
**Test status**: 待回归
**Changed**:
- `ConversationResponseView.swift`:`ConversationWorkAssistantRow` 降级为 **13.5pt + 0.8 透明度**;最终回复保持 15pt 主色。展开工作过程时过程叙述不再与结论同等抢眼。
- `SettingsView.swift`:「显示工作过程」→「**工作过程默认展开**」,文案说明:关闭(推荐,对齐 Codex)时回合只显示一行摘要、点箭头展开;开启后过程细节全量展开、消息明显变长。
- 版本号 0.5.255 → 0.5.256(四源同步)。

**Why**: 用户反馈完成态消息"看着辛苦";定位到两个叠加因素 —— 该开关开启使过程细节全量展开,且同一开关关闭时模型才被要求省略过程播报(`ResponsePersonalization`);同时过程消息与最终回复同样式、缺视觉分层。
**Next**: 若仍偏长,可评估废弃该开关、固定走 Codex 的完成态收起行为。
...
**Date**: 2026-09-10
**Tag**: v0.5.236
**Test status**: 全量回归 3144 通过(13 失败为存量远端集成环境问题,与改动无关)
**Changed**:
- `TaskInteractionViews.swift`:`TaskGoalCard` + `TaskQueueCard` 合并为 `TaskActivityStrip`——排队行与目标行同处一张卡,排队在上、目标行最下贴近 composer;目标行单行内联(转圈图标 + 粗体「进行中的目标」+ 截断文本 + 计时 + 垃圾桶/暂停/折叠角标);支持整条折叠为摘要行。
- `ChatView.swift`:排队行改 Codex 式——`text.append` 头图 + 纯文本迷你预览缩略图 + 单行文本 + 「⤷ 调整方向」带文字按钮 + 垃圾桶 + ⋯ 菜单(编辑/清空);计时格式改「11h 39m 48s」(带空格,小时位含秒);移除「待发送 · N 条」折叠头与 -25 叠压 hack。

**Why**: 旧目标卡(标题行+两行文本+展开)与排队卡(独立折叠头)与 Codex 实机差异大,用户给截图要求对齐。
**Next**: 继续对齐 Codex 桌面端其他细节。

## v0.5.235 — chore: 版本号整理（v0.5.232 → v0.5.235）
**Date**: 2026-09-10
**Tag**: v0.5.235
**Test status**: 全量回归 3145 通过 + 三机安装
**Changed**:
- 版本号清理。全部修复（helper 签名 / 问号输入 / 流光 / 电脑操作 chip）已包含。

**Why**: v0.5.232/234 版本号跳变造成混乱。
**Next**: 继续对齐 Codex 桌面端其他细节。
...
**Test status**: 全量回归 3143 通过 + 三机安装
**Changed**:
- TurnPresentationTests: .command 活动态文案含「正在运行 cmd」断言。

**Why**: 防止 v0.5.214 改动被无意回退。
**Next**: 继续对齐 Codex 桌面端其他细节。
...
**Date**: 2026-09-10
**Tag**: v0.5.226
**Test status**: 全量回归 3139 通过 + 三机安装
**Changed**:
- TurnPresentationTests：`.command` 完成态文案「已运行 cmd」断言。

**Why**: 防止 v0.5.219 改动被无意回退。
**Next**: 继续对齐 Codex 桌面端其他细节。
...
**Date**: 2026-09-10
**Tag**: v0.5.210
**Test status**: 全量回归 3120 通过 + 三机安装
**Changed**:
- DurationFormatter：输出「5 秒」「1 分 5 秒」「1 小时 2 分 5 秒」；中间零值省略。

**Why**: 对照 Codex 实机会话「已工作 43 秒」「用时 11 分钟 10 秒」。
**Next**: 继续对齐 Codex 桌面端其他细节。
...
v0.5.4 及更早版本见 `evolution/archive/EVOLUTION-pre-0.5.5.md`；不参与主日志版本校验。

## iOS 版本序列（独立编号）

iOS 1.0.x 版本记录已分离到 `evolution/ios/EVOLUTION.md`。
iOS 版本不参与 Mac 0.x tag、Info.plist 与 makeHistory 校验。
