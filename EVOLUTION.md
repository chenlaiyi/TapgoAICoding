...
## v0.5.4 — 用户截图持久缩略图、运行中逐步输出与精简上下文 UI
**Date**: 2026-08-28
**Commit**: _(see `git log -1 v0.5.4`)_
**Tag**: v0.5.4
**Test status**: — 720 passed, 0 failed —
**Changed**:
- 发送图片复制到 App 专属附件目录并随 `Turn` 持久化，用户消息中显示可恢复的缩略图；删除会话时同步清理附件。
- 运行中的回合不再隐藏已收到的 assistant、命令、工具和文件事件；命令/工具开始及结束都会立即产生两行进度。
- 删除会话列表的上下文百分比，以及输入框右下角的 context 进度条；Harness 自动压缩机制保持不变。
**Why**: 旧 UI 只把图片交给 Harness、没有写入会话模型；运行分支又主动隐藏中间 items，造成图片发送后消失、进度只在结束时集中出现。上下文百分比则把可自动压缩的动态窗口误导成硬上限。
**Next**: 用安装版完成截图发送后的用户消息缩略图、运行中开始/完成消息、重启恢复和两处 context UI 消失的原生回归。
