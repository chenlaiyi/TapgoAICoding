...
- 本机版本漂移（EVO-055）：发布不重启本机 App（重启会终止会话），于是「/Applications 已新版、运行进程还是旧版」会长期存在。漂移真源是 `scripts/evolution-drift.py`：`evolve.sh` 每轮算 `releasesBehind`（`evolution/versions/` 记录里 (running, installed] 的条数；running 早于最早记录时给 `null` 而非错数）、跨轮继承 `firstSeenAt`、`seenRuns` 按 run-id 去重，结果写进 `state.localApp.drift`，并把重启命令放进 `nextActions`（下一轮一读 state 就知道还欠这个动作）。随时查询：`python3 scripts/evolution-drift.py check`（有漂移打印一行并退出 1）。
