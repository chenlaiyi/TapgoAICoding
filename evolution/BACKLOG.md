...
- [x] **EVO-031 手机端回滚演练状态**：H5 自进化卡片显示最近 drill 的 PASS/FAIL/tag/时间（v0.5.286）
- [x] **EVO-032 月度维护自动化**：launchd 每月跑回滚演练 + 指标归档，成功静默、失败通知（v0.5.287）
...
- [x] **EVO-041 自进化成本归属**：App 把自进化会话的累计 token 写进 `state/evolution_cost.json`，evolve.sh 取增量并把来源记进 `costSource`（v0.5.297）
