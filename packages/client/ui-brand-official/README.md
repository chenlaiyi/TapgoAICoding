...
Mount this plugin in the browser roster of a Tapgo deployment, then build the client with the `official` profile so its occupants register.

### Choosing the profile

`DSH_CLIENT_BUILD_PROFILE` selects which brand renders. An `official` build shows the Tapgo icon and name in the sidebar and the Tapgo icon in the conversation hero; any other value leaves the shell fallbacks in place. The plugin still loads and validates in both cases; only the registration is profile-gated.

### Replacing the brand

A deployment with another identity leaves this package out and composes a package that occupies the sidebar and hero slots. Occupying a slot is the only composition route; there is no brand configuration surface here.
