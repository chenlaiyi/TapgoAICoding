...
在 Tapgo 部署的浏览器插件名单中挂载本包，并以 `official` profile 构建客户端。

### 选择 profile

`DSH_CLIENT_BUILD_PROFILE` 决定渲染哪个品牌。`official` 构建在侧栏与会话首屏显示 Tapgo 图标，侧栏同时显示产品名称；其他取值保留外壳的回退内容。两种情况下插件都会照常加载并通过校验；只有注册受 profile 门控。

### 替换品牌

其他品牌的部署不组合本包，而是组合占据侧栏和首屏 slot 的品牌包。占据 slot 是唯一的组合路径；这里不存在任何品牌配置面。
...
侧栏的两个填充作为一组声明感知的注册安装：嵌套的 `ctx.slots.inject()` 调用等待侧栏声明，因此无论本行在声明者之前还是之后激活，这组注册都能工作；声明消失时两个填充一并撤回，HMR 期间也不会留下残缺的品牌混合。浏览器半部是 [`src/client/index.ts`](src/client/index.ts)；node 半部是一个空 Loader 座位。浏览器标题是构建环境的事（`DSH_CLIENT_TITLE`），不在 slot 系统之内。
