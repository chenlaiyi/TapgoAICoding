...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('Tapgo AICoding')
  expect(labels(native.menus[0]!)).toEqual(['Open Tapgo AICoding', 'separator', 'Quit Tapgo AICoding'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 Tapgo AICoding', 'separator', '退出 Tapgo AICoding'])
  f.tray.dispose()
  f.tray.dispose()
