...
import { resolveDesktopAppId, resolveMacOSNotarizationEnvironment, resolveMacOSSigningEnvironment, resolveNpmRegistry, skipTapgoNotarization } from './desktop-release-environment.mjs'
...
const SHARED_SETTING = /^(?:TAPGO_DESKTOP_(?:NO_POLICY|RELEASE_VERSION|SKIP_NOTARIZATION)|DSH_DESKTOP_(?:APP_ID|AUTO_UPDATE_ENV|NPM_REGISTRY|MANDATORY_UPDATE_(?:CONFIG|(?:TEST|PROD)_ORIGIN))|DOWNLOAD_TEST_RELEASE_ID|DOWNLOAD_(?:TEST|PROD)_(?:ORIGIN|FEED_URL|COS_BUCKET|COS_SECRET_ID|COS_SECRET_KEY))$/u
...
const AMBIENT_RELEASE_SETTING = /^(?:TAPGO_DESKTOP_(?:NO_POLICY|RELEASE_VERSION|SKIP_NOTARIZATION)|DSH_DESKTOP_(?:APP_ID|AUTO_UPDATE_ENV|MANDATORY_UPDATE_.*|WINDOWS_.*|MACOS_.*)|APPLE_.*|(?:WIN_)?CSC_.*|DOWNLOAD_(?:TEST|PROD)_.*)$/iu
...
export function validateDesktopPackageEnvironment(environment, target, options = {}) {
  ...
  const skipNotarization = target.platform === 'darwin' && skipTapgoNotarization(environment)
