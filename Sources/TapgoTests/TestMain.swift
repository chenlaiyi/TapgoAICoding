...
let allSections: [String] = [
    ...
    "Sidebar: presentation and persistence",
    "Conversation: quiet work and readable responses",
    ...
    "PluginCatalog: Tapgo plugins.toml",
...
struct TapgoTestMain {
    ...
    static func runAll(runner: TestRunner) async {
        ...
        await runIfInScope(runner, "Evolution: cost attribution") {
            runEvolutionCost(runner)
        }
        ...
        await runIfInScope(runner, "Response: quiet transcript and personalization") {
            runResponsePresentationTests(runner)
        }
        await runIfInScope(runner, "TurnPresentation: semantic activity summaries") {
            runTurnPresentationTests(runner)
        }
        await runIfInScope(runner, "Conversation: quiet work and readable responses") {
            runConversationPresentationTests(runner)
        }
        await runIfInScope(runner, "Sidebar: presentation and persistence") {
            runSidebarPresentationTests(runner)
        }
        ...
        await runIfInScope(runner, "PluginCatalog: Tapgo parsing") {
            runPluginCatalogTapgoParsing(runner)
        }
        await runIfInScope(runner, "PluginCatalog: Tapgo safe pluginId") {
            runPluginCatalogTapgoSafeId(runner)
        }
        await runIfInScope(runner, "PluginCatalog: Tapgo plugins.toml") {
            runTapgoPluginsToml(runner)
        }
