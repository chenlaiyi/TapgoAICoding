...
func runDurationFormatter(_ t: TestRunner) {
    ...
    t.expectEqual(DurationFormatter.string(seconds: 65), "1 分钟 5 秒", "fmt: minutes")
    t.expectEqual(DurationFormatter.string(seconds: 3600), "1 小时", "fmt: hour")
    t.expectEqual(DurationFormatter.string(seconds: 3725), "1 小时 2 分钟 5 秒", "fmt: hour+min+sec")
    t.expectEqual(DurationFormatter.string(seconds: -3), "0 秒", "fmt: negative → 0s")
    // v0.5.210: 中间零值省略（对齐 Codex "1 小时" / "1 小时 5 秒" / "1 小时 2 分"）。
    t.expectEqual(DurationFormatter.string(seconds: 3660), "1 小时 1 分钟", "fmt: hour+min no sec")
