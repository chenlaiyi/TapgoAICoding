...
func runAppUpdateDistribution(_ t: TestRunner) {
    ...
    t.expect(sidebar.contains("menuItem(\"检查更新\""), "update: 左上角检查更新按钮")
    t.expect(sidebar.contains(".disabled(!updater.canCheckForUpdates)"), "update: 按钮跟随可检查状态")

    t.expectEqual(info["CFBundleShortVersionString"] as? String, "0.5.57", "update: 主 App 版本")
    t.expectEqual(helperInfo["CFBundleShortVersionString"] as? String, "0.5.57", "update: Helper 版本")
    ...
    t.expect(appcast.contains("Tapgo-AICoding-0.5.57.zip"), "update: appcast 指向当前归档")
