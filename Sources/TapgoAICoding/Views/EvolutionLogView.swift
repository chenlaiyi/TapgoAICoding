        ...
        return [
                EvolutionEntry(
                    ...
                    changes: [
                        ...
                        "HarnessDaemonConcurrencyTests + TomlKeyTests + ProviderRegistryPruneTests 新增并注册 (8 + 14 + 10 断言)。",
                        "AppBuilder/Info.plist: SUPublicEDKey 对齐实际发布签名密钥 (旧值不匹配 → Sparkle 签名校验一直失败, 自动更新从未生效; 0.5.318 及更早需手动升级一次)。",
                ...
                EvolutionEntry(
                    ...
                    changes: [
                        "新增 TapgoCore.EvolutionCostSnapshot:state/evolution_cost.json 的写入/读取契约(原子写、缺失或非法 JSON 都返回 nil),只记能核实的累计 token,不估算美元金额。",
                        "SessionStore 在自进化线程(evolutionMode 或 evo- 前缀)收到 turnCompleted/tokenUsageUpdated 时写入累计 usageTotal,同一值不重写、5 秒内且增量不足 1000 也跳过,写失败不影响会话主流程。",
                        ...
                        "state schema 升到 v4:新增 costSource 字段,说明 tokens/costUSD 来自哪条路径。",
                        "测试:Swift 新增 14 项成本契约断言(往返/覆盖/缺字段/非法 JSON),失败注入扩到 145 项(快照增量归因、快照未更新不归因、env 优先)。"
                    ],
                ...
                EvolutionEntry(
                    ...
                    changes: [
                        ...
                        "evolve.sh 在环境预检后立即执行 schema gate(--publish --resume 同样),失败 exit 13 且零改动;EVOLVE_SKIP_SCHEMA_CHECK=1 可跳过;benchmark 的 state-schema 检查改为调用该工具。",
                        "新增 32 项 schema 回归(补章/幂等/未来版本/类型错误/损坏行/必备字段/空目录/JSON 输出)+9 项失败注入断言(未来版本拒跑、缺章自动补后照常发布)+2 项 Swift 宽容读断言;真实 state 已补章 51 条记录。"
                    ],
                ...
                EvolutionEntry(
                    ...
                    changes: [
                        ...
                        "失败注入矩阵扩到 106 项:新增 release/push/worktree 三类续跑、已发布状态空操作、--resume 缺 --publish 拒绝等 27 项断言。"
                    ],
