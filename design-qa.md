# ZCode 桌面工作区复刻验收（v0.5.58）

- 参考图：`artifacts/zcode-desktop-reference/main.png`（ZCode 3.10.1，871 × 768 px）。
- 实现图：`artifacts/zcode-desktop-reference/tapgo-active-v0.5.58.png`（Tapgo AICoding 0.5.58，945 × 768 px）。
- 并排图：`artifacts/zcode-desktop-reference/zcode-vs-tapgo-v0.5.58.png`（同高度原图直接横向拼接，未拉伸）。
- 状态：通过（2026-08-31，本机 Developer ID 签名 Release App）。

## 并排验收结论

- P0：0；没有阻断任务创建、任务选择、输入或发送的问题。
- P1：0；侧栏、任务顶栏、消息区和输入器均无裁切、重叠、错位或关键交互缺失。
- P2：0；最终轮已消除输入器重复项目入口及系统蓝色任务选中态，选中行改为 ZCode 式中性灰。
- 侧栏层级与参考一致：四个一级入口、分组/项目切换、工具按钮、项目与任务树、相对日期、底部账户区。
- 主区层级与参考一致：紧凑任务标题、项目胶囊、更多操作、访达/终端/帮助/轨迹栏、居中消息和底部两层输入器。
- 颜色、密度、圆角、字号和暗色层级接近参考；图标均为 SF Symbols，没有占位图、拉伸素材或手绘替代物。
- 内容文本、项目数量与任务状态来自两款 App 各自真实数据，属于有意数据差异，不作为视觉缺陷。
