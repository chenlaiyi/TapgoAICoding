...
# artifacts/fidelity-vs-tapgo-0.5.75/fidelity-report.{json,md}.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT_DIR="$ROOT/artifacts/fidelity-vs-tapgo-0.5.75"
mkdir -p "$OUT_DIR"

DSH="$ROOT/Sources/TapgoAICoding/Resources/DSHTheme.swift"
TAPGO_PNG="$OUT_DIR/tapgo-main.png"
ZCODE_PNG="$OUT_DIR/01-baseline.png"
...
warn_ref=$(grep -A1 'warnZCode' "$DSH" | grep -oE '0x[0-9A-F]{6}' | head -1)
error_ref=$(grep -A1 'errorZCode' "$DSH" | grep -oE '0x[0-9A-F]{6}' | head -1)
...
node /tmp/qp-region.cjs "$ZCODE_PNG" > "$OUT_DIR/reference-region-colors.txt"
...
EOF
node /tmp/diff-region.cjs "$OUT_DIR/reference-region-colors.txt" "$OUT_DIR/tapgo-region-colors.txt" > "$OUT_DIR/region-color-diff.md"
...
cat > "$REPORT_JSON" <<EOF
{
  ...
  "metrics": {
    "color_token_precision": {
      ...
      "warnZCode":        { "expected": "0xCD8900",  "actual": "$warn_ref",       "match": "$([ "$warn_ref"  = "0xCD8900"  ] && echo true || echo false)" },
      "errorZCode":       { "expected": "0xE40014",  "actual": "$error_ref",      "match": "$([ "$error_ref" = "0xE40014"  ] && echo true || echo false)" }
    },
  ...
  "evidence_artifacts": [
    "01-baseline.png (ZCode 3.10.2 主窗口 1220x1287)",
    "tapgo-main.png (Tapgo AICoding v0.5.75 主窗口 963x1344)",
    "01-sidebar-droptarget.png + 02-main-bottom-error.png + 03-toolbar-status.png (ZCode 区域切图)",
    "diff-overlay.png (pixelmatch 红蓝叠加差图)",
    "pixelmatch.json (pixelmatch 详细输出)",
    "reference-region-colors.txt + tapgo-region-colors.txt (区域色采样)",
...
| warnZCode | \`0xCD8900\` | \`$warn_ref\` | $([ "$warn_ref" = "0xCD8900" ] && echo ✅ || echo ❌) |
| errorZCode | \`0xE40014\` | \`$error_ref\` | $([ "$error_ref" = "0xE40014" ] && echo ✅ || echo ❌) |
...
\`\`\`
$(cat $OUT_DIR/reference-region-colors.txt)
\`\`\`
...
- \`01-baseline.png\` — ZCode 3.10.2 主窗口 1220×1287（screencapture -l<wid> 抓取 wid=64377）
- \`tapgo-main.png\` — Tapgo AICoding v0.5.75 主窗口 963×1344（screencapture -l64836 抓取）
- \`01-sidebar-droptarget.png\` / \`02-main-bottom-error.png\` / \`03-toolbar-status.png\` — ZCode 主窗口的 3 个区域切图（brandBlueZCode / errorZCode / warnZCode 挂载点对照区）
...
echo "[reference-fidelity-report] wrote $REPORT_JSON and $REPORT_MD"
